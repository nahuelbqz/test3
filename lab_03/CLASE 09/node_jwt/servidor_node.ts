...
app.set("key", "cl@ve_secreta");//clave para generar el jwt
...
const verificar_jwt = express.Router();//generamos la ruta
    ...
    if(token){
        ...
        jwt.verify(token, app.get("key"), (error:any, decoded:any)=>{
            ...
            if(error){
                ...
                response.jwt = decoded;//le puedo pasar cualquier tipo de contenido al response
                //SE INVOCA AL PRÓXIMO CALLEABLE
                next();//va al verificar_token
            }
        });
    }
});
...
app.post("/crear_token", (request:any, response:any)=>{
    ...
    if((request.body.usuario == "admin" || request.body.usuario == "user") && request.body.clave == "123456"){
        ...
        const token = jwt.sign(payload, app.get("key"), {
            expiresIn : "1d"
        });//firmo el token, le paso payload y la contraseña

        response.json({
            mensaje : "JWT creado",
            jwt : token
        });//creo el json para mandarlo como respuesta(retorno)
    }
    else{
        response.json({
    ...
    request.getConnection((err:any, conn:any)=>{
        ...
        conn.query("select * from usuarios where legajo = ? and apellido = ? ", [obj.legajo, obj.apellido], (err:any, rows:any)=>{
            ...
            if(rows.length == 1){
                ...
                response.obj_usuario = rows[0];//agrego la fila del usuario a la respuesta
                //SE INVOCA AL PRÓXIMO CALLEABLE
                next();
            }
            else{
